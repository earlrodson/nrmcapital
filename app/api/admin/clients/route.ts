...
import { createClientSchema } from "@/lib/validations/api"
...
export async function GET(request: Request) {
  return withServerError(async () => {
    ...
    const result = await adminRepository.listClients({
      page: paging.page,
...
export async function POST(request: Request) {
  return withServerError(async () => {
    ...
    const { data, error } = await parseJsonWithSchema(request, createClientSchema)
