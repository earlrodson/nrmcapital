...
const TERMS = [3, 6, 9, 12, 15, 17] as const
...
export function LoanEstimator() {
  ...
  const [term, setTerm] = useState<(typeof TERMS)[number]>(6)
  ...
  return (
    <section className="mb-16 w-full max-w-md rounded-[2rem] border border-slate-100 bg-white p-6 shadow-2xl md:p-8">
      ...
      <p className="mb-6 text-center text-xs text-slate-500">
        Based on NRM Lending&apos;s flat-rate term matrix from the internal development specification.
      </p>
      ...
      <div className="mb-6">
        ...
        <div className="flex gap-1 rounded-xl bg-slate-100 p-1" role="radiogroup" aria-label="Loan term options">
          {TERMS.map((termValue) => {
            const active = termValue === term
            return (
              <button
                key={termValue}
                type="button"
                role="radio"
                aria-checked={active}
                onClick={() => setTerm(termValue)}
                className={`flex-1 rounded-lg py-2 text-sm font-bold transition focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-1 ${
      ...
      <p className="mt-3 text-center text-[11px] text-slate-400">
        Estimates are for reference only. Final approval, rates, and release terms remain subject to NRM credit evaluation.
      </p>
    </section>
  )
}
