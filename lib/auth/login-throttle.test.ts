...
describe("login throttle boundary", () => {
  it("allows the Nth failure without locking", () => {
    ...
    for (let i = 0; i < LOGIN_THROTTLE_CONFIG.identifier.maxAttempts - 1; i++) {
      state = nextThrottleStateAfterFailure(state, now)
    }
    expect(state!.failCount).toBe(LOGIN_THROTTLE_CONFIG.identifier.maxAttempts - 1)
  ...
  it("locks on the N+1th failure", () => {
    const now = new Date("2026-01-01T00:00:00.000Z")
    let state = null as ReturnType<typeof nextThrottleStateAfterFailure> | null
    for (let i = 0; i < LOGIN_THROTTLE_CONFIG.identifier.maxAttempts; i++) {
      state = nextThrottleStateAfterFailure(state, now)
    }
    expect(state!.failCount).toBe(LOGIN_THROTTLE_CONFIG.identifier.maxAttempts)
  ...
  it("stays locked for a request within the lock window", () => {
    ...
    const later = new Date(now.getTime() + LOGIN_THROTTLE_CONFIG.identifier.lockMs - 1)
  ...
  it("unlocks once the lock window has fully elapsed", () => {
    ...
    const after = new Date(now.getTime() + LOGIN_THROTTLE_CONFIG.identifier.lockMs + 1)
  ...
  it("resets the failure count once the tracking window has expired without a lock", () => {
    ...
    const afterWindow = new Date(now.getTime() + LOGIN_THROTTLE_CONFIG.identifier.windowMs + 1)
